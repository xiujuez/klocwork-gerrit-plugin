...
public class DefaultValuesTest implements BaseConfigTest {
    ...
    public static final String SEVERITY = "Review";
    public static final boolean NEW_ONLY = false;
    public static final boolean CHANGED_ONLY = false;
    public static final String COMMENT = "<severity> Klocwork violation:\n\n\n<message>\n\n\nRead more: <rule_url>";
    public static final String TITLE_NO_ISSUES = "Klocwork violations have not been found.";
    public static final String TITLE_ISSUES = "<total_count> Klocwork violations have been found.";
    ...
    public void testInspectionConfig() {
        Assert.assertEquals(KLOCWORK_URL, InspectionConfig.DescriptorImpl.KLOCWORK_URL);
        Assert.assertEquals(KLOCWORK_REPORT_PATH, SubJobConfig.DescriptorImpl.KLOCWORK_REPORT_PATH);
        ...
        Assert.assertEquals(KLOCWORK_REPORT_PATH, sConfig.getKlocworkReportPath());
        Assert.assertEquals(PROJECT_PATH, sConfig.getProjectPath());
    }

    @Test
    public void testBuildResultConfig() {
        Assert.assertEquals(MARK_FAILURE, BuildResultConfig.DescriptorImpl.MARK_FAILURE);

        BuildResultConfig config = new BuildResultConfig();
        Assert.assertEquals(MARK_FAILURE, config.getMarkFailure());
    }
}
